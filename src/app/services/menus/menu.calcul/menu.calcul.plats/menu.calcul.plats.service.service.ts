...
import { Cetape, Etape } from '../../../../../app/interfaces/etape';
...
export class MenuCalculPlatsServiceService {
  ...
  getPrimCost(prop: string, restaurant: string, plat: Cplat, _preparations: Array<Cpreparation>, _consommables: Array<Cconsommable>,
    ...
    let prepa_etapes: Array<Etape | null> = [];
    ...
    if (plat.preparations !== null) {
      const tmp_prepa = plat.preparations;
      let preparations = _preparations.filter((preparation) => tmp_prepa.flatMap((_preparation) => _preparation.id).includes(preparation.id));
    ...
    let ings = _ingredients.filter((ingredient) => arr_ings.flatMap((ingredient) => ingredient.id).includes(ingredient.id));
    ...
    let consos = _consommables.filter((consommable) =>  consommables.flatMap((_consommable) => _consommable.id).includes(consommable.id));
    return this.prepa_service.getPrimCost(etapes as Array<Cetape>, ings, arr_ings, consos);
  }
  getFullTheoTimeFromSec(plat: Cplat, preparations: Array<Cpreparation>): string {
    let sum_time_prepa = 0;
    let sum_time_plat = 0;
    let _preparations = preparations.filter((preparation) => preparations.map((preparation) => preparation.id).includes(preparation.id));
    if (plat.etapes !== null) {
      sum_time_plat = this.prepa_service.getFullTheoTimeSec(plat.etapes as Array<Cetape>)
    }
    if (plat.preparations !== null) {
      sum_time_prepa = _preparations.map((preparation) => {
        if ((preparation.etapes !== null) && (preparation.etapes !== undefined)) {
          ...
          if (etapes.length > 0) {
            return preparation.etapes.map((etape) => etape.time)
  ...
  getFullTheoTimeToSec(plat: Cplat, preparations: Array<Cpreparation>): number {
    ...
    if (plat.preparations !== null) {
      if(plat.preparations.length > 0){
        let plat_preparation = plat.preparations;
        plat_preparation = plat.preparations.filter((prepa) => (prepa !== undefined) && (prepa !== null))
        let _preparations = preparations.filter((preparation) => plat_preparation.flatMap((prepa) => prepa.id).includes(preparation.id));
          ...
          sum_time_prepa = _preparations
            .map((preparation) => {
              if (preparation.etapes !== null) {
                ...
                if (etapes.length > 0) {
                  const time_etapes = etapes.map((etape) => etape.time)
                  if (time_etapes !== null) {
  ...
  getPortionCost(plat: Cplat, preparations: Array<Cpreparation>, ingredients: Array<CIngredient>): number {
    ...
    if (plat !== null) {
      ...
      if (plat.preparations !== null) {
        const plat_preapartions = plat.preparations;
  ...
  platsRecommendationStep1(portion_cost: number | null): number {
